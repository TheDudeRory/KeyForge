//! Global Hotkeys — the engine + JSON profile model, with the UI in React
//! (src/components/HotkeyManager.tsx). Registration goes through
//! tauri-plugin-global-shortcut rather than the raw global-hotkey crate, so
//! there is exactly one OS shortcut owner in the process (lib.rs owns the two
//! app-level shortcuts on the same plugin).
//!
...
use crate::childenv::strip_webview_env;
use serde::Serialize;
