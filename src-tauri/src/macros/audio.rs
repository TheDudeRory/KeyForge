...
pub mod pactl_impl {
    ...
    pub fn pactl_command() -> Command {
        ...
        // Single choke point for every pactl spawn, so the display-backend env
        // we set for the webview never reaches it (see childenv::strip_webview_env).
        crate::childenv::strip_webview_env(&mut cmd);
        cmd
    }
