...
#[cfg(not(windows))]
pub fn open_path(path: &str) -> Result<(), String> {
    // xdg-open resolves the user's handler from their session, so it must not
    // inherit the X11 backend we pin for the webview.
...
pub async fn run_shell(
    ...
    // The user's shell must see the user's session, not our webview's env.
